...
void MAIN_CHAR::updateViewDirection(float xRel, float yRel)
{
	mSceneNode->yaw(Ogre::Degree(-xRel * 0.25f));
	//mCamera->yaw(Ogre::Degree(xRel * 0.25f));
	//mSceneNode ->pitch(Ogre::Degree(-yRel * 0.25f));
}
...
void MAIN_CHAR::walkForward(const Ogre::FrameEvent& evt)
{
		...
		*/
		...
		Matrix3 kot;
		mSceneNode->getOrientation().ToRotationMatrix(kot);

		actualDirection = Vector3(0,0,-1) * kot;
		//actualDirection = mCamera->getDerivedDirection();
		Vector3 d;
		mSpeedFactor = 100;
